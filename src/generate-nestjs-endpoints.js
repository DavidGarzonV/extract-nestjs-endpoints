...
const PARAM_DECORATORS = 'Body|Param|Query|Req|Res|UploadedFiles|UploadedFile|Headers';
...
function skipDecoratorsAndModifiers(src, idx) {
  ...
  while (true) {
    ...
    const modMatch = /^(public|private|protected|static|readonly)\b/.exec(src.slice(i, i + 20));
    if (modMatch) {
      i += modMatch[0].length;
      continue;
    }
...
function extractFileFields(decoratorsBlock) {
  ...
  const fieldRegex = /\{\s*name:\s*['"]([^'"]+)['"]\s*(?:,\s*maxCount:\s*(\d+))?\s*\}/g;
  let fm;
...
function parseParams(paramsStr) {
  ...
  for (const arg of args) {
    const decoratorMatch = arg.match(new RegExp(`@(${PARAM_DECORATORS})\\s*(\\(([^]*)\\))?`));
    if (!decoratorMatch) {
      ...
      result.push({
        ...
        type: nameTypeMatch ? nameTypeMatch[3].trim().replace(/\s+/g, ' ') : undefined,
...
function extractMethods(src) {
  ...
  while ((match = decoratorRegex.exec(src)) !== null) {
    ...
    const { argsStr, nextIdx } = readDecoratorArgs(src, decoratorRegex.lastIndex);
    let routePath = '';
    if (argsStr) {
    ...
    const { nextIdx: afterDecoratorsIdx, skippedText } = skipDecoratorsAndModifiers(src, nextIdx);
    const asyncMatch = /^async\s+/.exec(src.slice(afterDecoratorsIdx, afterDecoratorsIdx + 10));
    const nameStart = asyncMatch ? afterDecoratorsIdx + asyncMatch[0].length : afterDecoratorsIdx;
    const nameMatch = /^([A-Za-z0-9_$]+)\s*\(/.exec(src.slice(nameStart, nameStart + 200));
...
function buildClassIndex(srcRoot) {
  ...
  for (const file of files) {
    ...
    const regex = /export\s+class\s+([A-Za-z0-9_]+)(?:\s+extends\s+([A-Za-z0-9_]+))?/g;
    let m;
...
function parseClassFields(body) {
  ...
  while ((m = fieldRegex.exec(cleaned)) !== null) {
    ...
    fields.push({ name: name, type: m[3].trim().replace(/\s+/g, ' '), optional: !!m[2] });
  }
  return fields;
}

function resolveDtoFields(className, classIndex, cache = new Map(), stack = new Set()) {
  ...
  if (p.destructured) {
    ...
    if (known) {
      return p.destructured.map((name) => {
        ...
        return found ? { name: found.name, type: found.type, optional: found.optional } : { name: name, type: 'any', optional: false };
      });
    }
    return p.destructured.map((name) => ({ name: name, type: 'any', optional: false }));
  }
  if (!p.key) {
    const known = resolveDtoFields(p.type, classIndex, dtoCache);
    if (known && known.length > 0) return known;
    return [{ name: '(whole object)', type: p.type || 'any', optional: false }];
  }
  ...
  for (const file of controllerFiles) {
    ...
    for (const m of methods) {
      ...
      if (isMultipart) {
        ...
        const textFields = bodyFields.filter((p) => p.key).map((p) => ({ field: p.key, type: p.type || 'any', format: 'json-string' }));
        ...
        if (wholeBody) {
          ...
          body = { contentType: 'application/json', schema: wholeBody.type || 'any', fields: known || undefined };
        } else {
          body = { contentType: 'application/json', fields: bodyFields.map((p) => ({ name: p.key, type: p.type || 'any' })) };
        }
      }

      const files = m.fileFields.map((f) => ({ field: f.campo, maxCount: f.maxCount }));
